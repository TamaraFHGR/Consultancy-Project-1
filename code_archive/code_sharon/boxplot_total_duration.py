...
# Durchschnittliche Dauer pro Person pro Stadt und Ausprägung (color oder grey)
mean_duration_per_person = df.groupby(['City', 'user', 'description'])['FixationDuration'].mean().reset_index()

# Aufteilen der Daten nach description (color oder grey)
color_data_mean = mean_duration_per_person[mean_duration_per_person['description'] == 'color']
grey_data_mean = mean_duration_per_person[mean_duration_per_person['description'] == 'grey']

# Erstellen der Boxplots für durchschnittliche Dauer
plt.figure(figsize=(12, 8))

# Boxplot für durchschnittliche Dauer für color
sns.boxplot(data=color_data_mean, x='City', y='FixationDuration', color='skyblue', width=0.4, linewidth=1.5, flierprops=dict(marker='o', markersize=5))

# Boxplot für durchschnittliche Dauer für grey
sns.boxplot(data=grey_data_mean, x='City', y='FixationDuration', color='lightgrey', width=0.4, linewidth=1.5, flierprops=dict(marker='o', markersize=5))
...
plt.title('Boxplot der durchschnittlichen Dauer pro Person pro Stadt (Color vs Grey)')
plt.xlabel('Stadt')
plt.ylabel('Durchschnittliche Fixationsdauer (ms)')
plt.xticks(rotation=45)
plt.grid(True)
